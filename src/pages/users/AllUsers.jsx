...
export default function AllUsers() {
  ...
  // ── Fetch users (status, sort, search, pagination — all server-side) ───────
  useEffect(() => {
    if (!baseURL || !token) return;
    ...
    fetchUsers();
  }, [baseURL, token, limit, offset, statusFilter, sortFilter, debouncedSearch]);
  ...
  const handleStatusChange = async (userId, newStatus) => {
    ...
    try {
      ...
      if (res.data?.status === "success") {
        setUsers((list) => list.map((u) => u.id === userId ? { ...u, status: newStatus } : u));
      }
    } catch (err) {
