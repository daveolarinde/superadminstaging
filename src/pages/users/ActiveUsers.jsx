...
export default function ActiveUsers() {
  ...
  // ── Fetch users (status locked to "active", sort, search, pagination — all server-side) ──
  useEffect(() => {
    if (!baseURL || !token) return;
  ...
  const handleStatusChange = async (userId, newStatus) => {
    ...
    try {
      ...
      if (res.data?.status === "success") {
        // Status changed away from "active" means it no longer belongs in this
