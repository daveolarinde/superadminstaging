...
export default function BlockedUsers() {
  ...
  // ── Fetch users (status locked to "blocked", sort, search, pagination — all server-side) ──
  useEffect(() => {
    if (!baseURL || !token) return;
  ...
  const handleStatusChange = async (userId, newStatus) => {
    ...
    try {
      ...
      if (res.data?.status === "success") {
        // Status changed away from "blocked" means it no longer belongs in this
        // list — drop it locally instead of just relabeling it in place.
        setUsers((list) =>
          newStatus === "blocked"
