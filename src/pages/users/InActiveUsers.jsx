...
export default function InactiveUsers() {
  ...
  // ── Fetch users (status locked to "inactive", sort, search, pagination — all server-side) ──
  useEffect(() => {
    if (!baseURL || !token) return;
  ...
  const handleStatusChange = async (userId, newStatus) => {
    ...
    try {
      ...
      if (res.data?.status === "success") {
        // Status changed away from "inactive" means it no longer belongs in
  ...
  return (
    <div className="p-4 md:p-6 bg-gray-50 min-h-screen space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold text-gray-900">Inactive Users</h1>
