...
const getCurrencyLabel = (currency = "NGN") => {
  const labels = {
    NGN: "Nigerian Naira",
    USD: "US Dollar",
  };
...
const fmt = (val, type, currency = "NGN") => {
  if (type === "percent") return `${parseFloat(val || 0).toFixed(2)}%`;
  return `${getCurrencySymbol(currency)}${Number(val || 0).toLocaleString("en-US", {
...
// ── Reward input ──────────────────────────────────────────────────────────────
const RewardInput = ({ label, value, onChange, rewardType, currency }) => {
...
const ReferralSettings = () => {
  ...
  const fetchSettings = useCallback(async () => {
    ...
    try {
      ...
      if (!current) {
        throw new Error("No referral settings found.");
      }

      const normalized = {
        ...current,
        transactionClass: current.transactionClass || "all",
  ...
  const handleSave = async () => {
    ...
    if (draft.rewardType === "percent") {
      if (
        parseFloat(draft.referrerReward) > 100 ||
        parseFloat(draft.referredReward) > 100
      ) {
  ...
  return (
    <div className="min-h-screen bg-gray-50/60 p-6">
      <div className="max-w-2xl mx-auto space-y-6">
              ...
              {[
                ...
                return (
                  ...
                  >
                    ...
                    {active && <Check size={14} className="text-blue-600 ml-auto shrink-0 mt-0.5" />}
                  </button>
                );
              })}
            </div>
        ...
        {/* ── Threshold, currency & transaction class ── */}
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
          <div className="px-5 py-4 border-b border-gray-50">
            <p className="text-sm font-bold text-gray-800">Threshold, Currency & Transaction Class</p>
            <p className="text-xs text-gray-400 mt-0.5">
              Minimum transaction amount and transaction type that qualify for referral reward
            </p>
          </div>
          <div className="p-5 grid grid-cols-3 gap-4">
            ...
            <div>
              ...
              <div className="flex items-center px-3.5 py-2.5 rounded-xl border border-gray-100 bg-gray-50 text-sm font-semibold text-gray-500">
                {draft?.currency} — {getCurrencyLabel(draft?.currency)}
              </div>
            </div>
            ...
            <div>
              ...
              <select
                value={draft?.transactionClass ?? "all"}
                onChange={e => update("transactionClass", e.target.value)}
                className="w-full px-3.5 py-2.5 rounded-xl border border-gray-200 bg-white text-sm font-medium text-gray-700 focus:outline-none focus:border-blue-400 focus:ring-2 focus:ring-blue-50 transition"
              >
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
          <div className="px-5 py-4 border-b border-gray-50">
            ...
            <p className="text-xs text-gray-400 mt-0.5">
              Control how and when referral rewards are applied
            </p>
          </div>
        ...
        {draft && (
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
            <div className="px-5 py-4 border-b border-gray-50">
              ...
              <p className="text-xs text-gray-400 mt-0.5">
                How the referral reward reads with current settings
              </p>
            </div>
            <div className="px-5 py-4 space-y-2">
              <p className="text-sm text-gray-600 leading-relaxed">
                ...
                <span className="font-semibold text-gray-800">
                  {draft.transactionClass === "all"
                    ? "qualifying transaction"
                    : draft.transactionClass}
                </span>{" "}
                transaction of at least{" "}
                <span className="font-semibold text-gray-800">
                  {getCurrencySymbol(draft.currency)}
                  {Number(draft.thresholdAmount || 0).toLocaleString()}
                </span>
                , the referrer receives{" "}
                <span className="font-semibold text-blue-600">
                  {fmt(draft.referrerReward, draft.rewardType, draft.currency)}
                </span>{" "}
                and the referred user receives{" "}
                <span className="font-semibold text-blue-600">
                  {fmt(draft.referredReward, draft.rewardType, draft.currency)}
                </span>
                .
                {draft.isOneTime && (
