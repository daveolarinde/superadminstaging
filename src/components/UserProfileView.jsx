    ...
    <div className="p-6 space-y-8 animate-fadeIn">
      ...
      <div className="bg-white shadow-sm border border-gray-100 rounded-2xl p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-8 text-sm">
          <Detail label="Country" value={user?.country || "N/A"} />
          <Detail label="Phone Number" value={user?.phoneNumber || "N/A"} />
          <Detail
            label="Status"
          ...
          />
          <Detail label="Address" value={parsedAddress} />
          <Detail
            label="Verification Status"
            value={user.isVerified ? "Verified" : "Unverified"}
          />
          <Detail label="ID Type" value={user.idType ? "nin" : "bvn"} />
          <Detail label="Gender" value={user.gender || "N/A"} />
          <Detail label="Date of Birth" value={user.dob || "N/A"} />
          <Detail
          ...
          <Detail
            ...
            value={user.backgroundInformation ? "Yes" : "No"}
          />
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <BalanceCard
          ...
          value={`₦${nairaBalance.toLocaleString()}`}
        />
        <BalanceCard
          title="Account Balance ($)"
          value={`$${usdBalance.toLocaleString()}`}
        />
      </div>
        ...
        {user?.cards?.length > 0 ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {user.cards.map((card) => (
              <div
                key={card.id}
                className="border border-gray-100 rounded-xl p-4 shadow-sm bg-gray-50 hover:bg-gray-100 transition"
              >
...
function Detail({ label, value, className = "" }) {
  return (
    <div>
      ...
      <p className={`font-medium text-gray-700 ${className}`}>{value}</p>
    </div>
  );
}
...
function BalanceCard({ title, value }) {
  return (
    <div className="bg-white border border-gray-100 rounded-2xl shadow-sm p-6 text-center md:text-left">
